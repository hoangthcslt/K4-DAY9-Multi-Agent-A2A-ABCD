...
The model name lives here in source (never in .env) because the grader reads it
from the code and from metadata.json. Only the API key comes from .env.
"""
...
def call_llm(system_prompt, user_prompt, temperature=DEFAULT_TEMPERATURE,
    ...
    json_mode asks Groq to constrain the reply to a JSON object, which keeps the
    small model's output parseable for agent handoff.
    """
    payload = {
        "model": MODEL_NAME,
        "messages": [
    ...
    for attempt in range(MAX_RETRIES):
        try:
            response = requests.post(
                GROQ_ENDPOINT, headers=headers, json=payload, timeout=60
            )
